...
;// CONCATENATED MODULE: ./src/constants/restrictedRoutes.ts
const WebUser = [
    "/apps/users/*"
];
const ApiUser = [
...
;// CONCATENATED MODULE: ./src/utils/auth/role-management.ts

const isLockedPath = (roles, path)=>{
    if (!roles.length) return [];
    const activeRoleRoutes = roles.map((x)=>restrictedRoutes[x]).filter((x)=>x);
    const lockedRoutesPath = activeRoleRoutes.reduce((acc, current)=>[
            ...acc,
            ...current
        ], []);
    const routeCount = lockedRoutesPath.reduce((acc, current)=>({
            ...acc,
            [current]: (acc[current] ?? 0) + 1
        }), {});
    let nbrRole = 0;
    roles.forEach((r)=>{
        if (restrictedRoutes[r]) nbrRole++;
    });
    const lockedPath = Object.entries(routeCount).filter(([, value])=>value >= nbrRole).map(([key])=>key);
    return lockedPath.every((x)=>!new RegExp(`^${x.replace("*", ".*")}$`).test(path));
};
...
/* harmony default export */ const src_middleware = ((0,middleware.withAuth)({
    callbacks: {
        authorized: ({ token, req: request })=>{
            ...
            const roles = token.decoded.resource_access[process.env.KEYCLOAK_CLIENT_ID ?? ""].roles;
            if (!roles || !roles?.length) return false;
            const path = new URL(request.url).pathname;
            return !!isLockedPath(roles, path);
        }
    },
    pages: {
        signIn: "/"
...
;// CONCATENATED MODULE: ./node_modules/next/dist/build/webpack/loaders/next-middleware-loader.js?absolutePagePath=private-next-root-dir%2Fsrc%2Fmiddleware.ts&page=%2Fsrc%2Fmiddleware&rootDir=C%3A%5CUsers%5Cmevaa%5CDesktop%5CWORK%5CRegBook%5CRegbookFront&matchers=W3sicmVnZXhwIjoiXig%2FOlxcLyhfbmV4dFxcL2RhdGFcXC9bXi9dezEsfSkpP1xcL2Rhc2hib2FyZCg%2FOlxcLygoPzpbXlxcLyNcXD9dKz8pKD86XFwvKD86W15cXC8jXFw%2FXSs%2FKSkqKSk%2FKC5qc29uKT9bXFwvI1xcP10%2FJCIsIm9yaWdpbmFsU291cmNlIjoiL2Rhc2hib2FyZC86cGF0aCoifSx7InJlZ2V4cCI6Il4oPzpcXC8oX25leHRcXC9kYXRhXFwvW14vXXsxLH0pKT9cXC9hcHBzKD86XFwvKCg%2FOlteXFwvI1xcP10rPykoPzpcXC8oPzpbXlxcLyNcXD9dKz8pKSopKT8oLmpzb24pP1tcXC8jXFw%2FXT8kIiwib3JpZ2luYWxTb3VyY2UiOiIvYXBwcy86cGF0aCoifV0%3D&preferredRegion=&middlewareConfig=eyJtYXRjaGVycyI6W3sicmVnZXhwIjoiXig%2FOlxcLyhfbmV4dFxcL2RhdGFcXC9bXi9dezEsfSkpP1xcL2Rhc2hib2FyZCg%2FOlxcLygoPzpbXlxcLyNcXD9dKz8pKD86XFwvKD86W15cXC8jXFw%2FXSs%2FKSkqKSk%2FKC5qc29uKT9bXFwvI1xcP10%2FJCIsIm9yaWdpbmFsU291cmNlIjoiL2Rhc2hib2FyZC86cGF0aCoifSx7InJlZ2V4cCI6Il4oPzpcXC8oX25leHRcXC9kYXRhXFwvW14vXXsxLH0pKT9cXC9hcHBzKD86XFwvKCg%2FOlteXFwvI1xcP10rPykoPzpcXC8oPzpbXlxcLyNcXD9dKz8pKSopKT8oLmpzb24pP1tcXC8jXFw%2FXT8kIiwib3JpZ2luYWxTb3VyY2UiOiIvYXBwcy86cGF0aCoifV19!
